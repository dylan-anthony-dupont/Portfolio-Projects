#!/usr/bin/env python3
"""
v33_generate_tail_certificate.py

Deterministically generates v33_tail_certificate_m10.json from v33_constants_m10.json using
directed-rounding interval arithmetic implemented with Python's decimal module.

This generator is intended to be auditable: no network access, no randomness, and no external libraries.

Tail inequality certified (for given inputs):
  LHS(delta) < RHS(delta), where
  LHS(delta) = 2*C_up*( delta^(3/2)*L(m) + delta^(1/2)*N_up(m)/kappa )
  RHS(delta) = c - delta*( Kalloc*c0*L(m) + C_hpp*(log(m)+1) )

with
  L(m)    = C1*log(m) + C2,
  N_up(m) = 1.01*log(m) + 17,
  c  = (3 ln 2)/16,
  c0 = (3 ln 2)/(8 pi),
  Kalloc = 3 + 8*sqrt(3).

Usage:
  python3 v33_generate_tail_certificate.py v33_constants_m10.json v33_tail_certificate_m10.json
"""
...
def iv(lo: str, hi: str | None = None) -> Interval:
    if hi is None:
        hi = lo
...
def compute(constants: dict, prec: int = 90) -> dict:
    ...
    # N_up(m) = 1.01*logm + 17
    Nup = add(mul(iv("1.01"), logm, prec), iv("17"), prec)
    ...
    # local term: delta^(1/2) * Nup / kappa
    local_term = mul(sqrt(delta, prec), div(Nup, kappa, prec), prec)

    # residual term: delta^(3/2) * L
    residual_term = mul(pow_3_2(delta, prec), L, prec)

    lhs = mul(mul(iv("2"), Cup, prec), add(residual_term, local_term, prec), prec)
    ...
    return {
        ...
        "Nup_interval": {"lo": str(Nup.lo), "hi": str(Nup.hi)},
        "kappa_interval": {"lo": str(kappa.lo), "hi": str(kappa.hi)},
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: v33_generate_tail_certificate.py constants.json tail_certificate.json", file=sys.stderr)
        return 2
