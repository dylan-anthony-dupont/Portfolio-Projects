...
\phantomsection %% [PATCH 3i.0-ANCHOR]
...
\(\displaystyle \big|\Real[\pi\cot(x+iy)]\big|=\pi\,\frac{|\sin(2x)|}{\cosh(2y)-\cos(2x)}\le \frac{\pi}{\cosh(2y)-1}\). %% [PATCH 3i.0-COT]
...
\end{equation}
\begin{proof} %% [PATCH 3i.0-GCHIPROOF]
By definition \(E(v)=\Lambda_2(1+v)\). Thus
\[
G(v)=\frac{E(1+v)}{E(1-v)}=\frac{\Lambda_2(2+v)}{\Lambda_2(v)}.
\]
The completed functional equation \(\Lambda_2(u)=\Lambda_2(2-u)\) implies
\[
\frac{\Lambda_2(2+v)}{\Lambda_2(v)}
=\frac{\Lambda_2(2-(2+v))}{\Lambda_2(2-v)}
=\frac{\Lambda_2(-v)}{\Lambda_2(2-v)}.
\]
Hence \(G(v)G(-v)=\dfrac{\Lambda_2(2+v)}{\Lambda_2(v)}\cdot\dfrac{\Lambda_2(2-v)}{\Lambda_2(-v)}\).
Using the non‑completed FE for \(\zeta_2\) in width–2, one obtains the modulus identity
\[
|G(v)|\,|G(-v)|=\big|\chi_2(1+v)\big|\,\big|\chi_2(1-v)\big|.
\]
Evaluating at \(v=a+im\) yields \eqref{eq:G-chi-product}. Any potential pole at \(v=0\) (the pole of \(\zeta\) at \(1\)) is avoided by the boundary‑contact convention (we never place \(\partial B\) through \(v=0\)).
\end{proof}
\end{lemma}
...
\end{equation}
and a compactly supported \(C^\infty\) weight \(W\in C_c^\infty([0,1])\) with \(\int_0^1 W=1\) (Appendix~\nameref{app:PW}). %% [PATCH 3i.0-NAMEREF-PW]
...
The following numbers are \textbf{illustrative} and reproducible; full data/scripts available on request. %% [PATCH 3i.0-ILLUSTRATIVE]
...
Complete \(j=1..50\) tables (predicted vs.\ true) for \((A,C)=(2.5,32)\) and \((2.5,48)\) are available in a companion repository (anonymized for review) and on request; filenames:
\begin{itemize}[leftmargin=1.2em]
...
\phantomsection %% [PATCH 3i.0-ANCHOR]
\addcontentsline{toc}{section}{Appendix PW. A concrete Paley--Wiener weight}
