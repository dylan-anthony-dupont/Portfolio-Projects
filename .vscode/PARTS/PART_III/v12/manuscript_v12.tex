...
\subsection*{7) Toolbox $\to$ structural consequences}
These become corollaries once $a(m)=0$ is proved (see Part~III).
...
\]
hence $f'(\sigma)<0$ on $\R$ for all such $t$. Since $f(1)=0$, $|\chi_2(u)|=1$ iff $\Real u=1$. For $|t|<m_1/2$ the range is covered by the certified base band (Appendix~\ref{app:firstheight-certified}).
\end{theorem}
...
Under \eqref{eq:rouche-ratio}, $\log|W|$ is harmonic with zero boundary trace, hence $|W|\equiv 1$ and $W\equiv e^{\ii\theta_B}$.
\end{proposition}

\begin{proposition}[Bridge~2: stitching]\label{prop:bridge2}
If $B_1,B_2$ overlap and $W\equiv e^{\ii\theta_{B_j}}$ on $B_j$, then phases agree on overlaps; a tiled band inherits a single unimodular phase.
\end{proposition}
...
\begin{remark}[Two “outers”]
$H$ denotes an \emph{outer majorant} for a general $G$ (used in Schur pinning); $\Gout$ denotes the \emph{modulus‑outer} of $E$ (used in Rouch\'e). Both are analytic and zero‑free; roles differ.
\end{remark}
...
\end{equation}
For a conservative bound one may set $K_{\rm alloc}^{\star}(\lambda):=2\big[(2-\lambda)+4\sqrt{2(2-\lambda)}\big]$.
\end{lemma}
...
\end{equation}
Consequently, no off-axis quartet lies in any $B(\alpha,m,\delta)$ for $m\ge M_0(\eta)$. Combined with a certified base range “no zeros below $m_1$” (Appendix~\ref{app:firstheight-certified}) and, when $M_0(\eta)>m_1$, certification of the finite band $[m_1,M_0(\eta)]$ via the Outer/Rouch\'e pipeline (Section~\ref{sec:criteria} and Appendix~\ref{app:cert}), all nontrivial zeros lie on $\Real s=\tfrac12$.
\end{theorem}

\paragraph{Choice of $M_0(\eta)$.}
A sufficient condition ensuring \eqref{eq:upper-lower-compare} for all $\alpha\in(0,1]$ is
...
\end{equation}
Since the left side is $o(1)$ and the right side $\to c_0\pi/4>0$, some $M_0(\eta)$ exists.

% ======================================================================
% Part III — Structural Corollaries (post-Theorem)
% ======================================================================
...
Assume the Main Theorem of Part~II: for every nontrivial height $m>0$, $a(m)=0$.

\begin{corollary}[Canonical columns]\label{cor:canonical-columns}
Define $\Podd(n)=(1-\cos\pi n)/2$ and $\Peven(n)=(1+\cos\pi n)/2$. Let $k:\Z\to\Z$ be $k(2j-1)=j$, $k(2j)=j+1$ (e.g.\ $k(n)=\tfrac{n}{2}+\tfrac{1-\cos\pi n}{4}$). For $x\in(0,2)$ set
\[
...
\]
Under $a(m)=0$ at each nontrivial height, $x=1$ yields $\UR(1,n)=\UL(1,n)$.
\end{corollary}
...
\end{equation}
and a Paley--Wiener weight $W\in C_c^\infty([0,1])$
with $0\le W\le 1$ and $\int_0^1 W(y)\,dy=1$ (Appendix~\ref{app:PW}).
...
\begin{corollary}[C3: Even--site curvature]\label{cor:C3}
...
\begin{corollary}[C4: Newton contraction]\label{cor:C4}
...
\begin{corollary}[C5: Canonical Weil weight]\label{cor:C5}
With $W=\widehat\phi\!\restriction_{[0,1]}$ for even $\phi\in C_c^\infty(\R)$, replacing $\Lambda(n)$ by prime powers yields the same forms and bounds.
\end{corollary}

\begin{theorem}[Prime--locked generator of $\{m_j\}$]\label{thm:generator}
Fix $W$ and $X_j=(\log t_j)^{3/2}$. Given $m_1$ (Appendix~\ref{app:firstheight-certified}) and the Main Theorem, define $m_{j+1}$ by
...
\;=\;\pi\,.
\end{equation}
Then for $j\ge j_0$ the solution is unique and obtained by damped Newton in $O(1)$ steps with contraction factor $1-\kappa/\log t_j$. The first finitely many indices are covered by the certified band of Part~II.
\end{theorem}
...
Monotonicity of $\log|\chi_2|$ follows from $\partial_\sigma\log|\Gamma|=\Real\psi$ and $\psi(1-z)-\psi(z)=\pi\cot(\pi z)$; the cosh-bound form appears in Theorem~\ref{thm:hinge}.
...
\begin{itemize}
\item Boundary meshes: interval bounds for $|E|$, $\arg E$ on $\partial B$ at side mesh $N_{\rm side}$.
...
\item Certificate: verify $\sup_{\partial B}|E-\Gout|/|\,\Gout|<1$.
\end{itemize}

\section{Toolbox (structural; not used in proofs)}
Catalog of auxiliary identities/filters (modulated families, ray curvature extractor). Not used in Section~\ref{sec:tail}.
...
\section{A concrete Paley--Wiener weight}\label{app:PW}
Let $\eta\in C^\infty(\R)$ be
\[
\eta(y)=\begin{cases}\exp\!\big(-1/(y(1-y))\big),&y\in(0,1),\\[2pt]0,&\text{elsewhere.}\end{cases}
\]
Set $W(y):=c_W\,\eta(y)$ on $[0,1]$ with $c_W:=\big(\int_0^1 \eta\big)^{-1}$ so $\int_0^1W=1$ and $0\le W\le c_W$.
