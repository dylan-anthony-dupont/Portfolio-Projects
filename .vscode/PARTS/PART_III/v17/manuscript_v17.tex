...
\usepackage{hyperref}
\usepackage{nameref} %% [PATCH 2.0-ENABLENAMEREF]
...
\end{equation}
\emph{Derivation:} expanding \(\Lambda_2\) at \(1\pm v\) yields the identity; poles at \(v=0\) (the pole of \(\zeta\) at \(1\)) are avoided by the boundary‑contact convention.
\end{lemma}
...
If \(B_1,B_2\) overlap and \(W\equiv e^{i\theta_{B_j}}\) on \(B_j\) \((j=1,2)\), then \(e^{i\theta_{B_1}}=e^{i\theta_{B_2}}\) on \(B_1\cap B_2\). %% [PATCH 2.0-DELIMS]
\end{proposition}
...
\addcontentsline{toc}{section}{Part III (continued) --- Prime--Locked Corollaries and Generator} %% [PATCH 2.0-TOC-P3CONT]
...
\phantomsection
\addcontentsline{toc}{section}{Appendix S.1. Operator norms on Lipschitz boundaries (shape-only dependence)} %% [PATCH 2.0-TOC-APP]
...
\phantomsection
\addcontentsline{toc}{section}{Appendix S.2. Instantiating $(C_1,C_2)$ from explicit literature bounds (optional)} %% [PATCH 2.0-TOC-APP]
...
\phantomsection
\addcontentsline{toc}{section}{Appendix S.3. Pinned constants closing the band} %% [PATCH 2.0-TOC-APP]
Choose
\[
...
\phantomsection
\addcontentsline{toc}{section}{Appendix PW. A concrete Paley--Wiener weight} %% [PATCH 2.0-TOC-APP]
